...
/**
 * Created by Ashok Kumawat on 5/25/2018.
 */
        ...
        if (text.toString().contains("*")) {
            ...
            if (doubleStars >= 2) {
                ...
                for (int i = 0; i < doubleStars; i++) {
                    text = text.toString().replaceFirst(Pattern.quote("**"), bool ? "<u>" : "</u>");
                    bool = !bool;
                }
            }
        }
        ...
        if (text.toString().contains("_")) {
            ...
            if (underscores >= 2) {
                ...
                for (int i = 0; i < underscores; i++) {
                    text = text.toString().replaceFirst(Pattern.quote("_"), bool ? "<i>" : "</i>");
                    bool = !bool;
                }
            }
        }

        text = text.toString().replaceAll(Pattern.quote("<p dir=\"ltr\">"), "").replaceAll(Pattern.quote("</p>"), "").replaceAll("\n", "<br>");
