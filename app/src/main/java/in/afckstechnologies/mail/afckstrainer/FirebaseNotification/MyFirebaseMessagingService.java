...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private void sendPushNotification(JSONObject json) {
        ...
        try {
            ...
            if(title.length()>13) {
                 buzzMsg = title.substring(0, 13);
            }
            //creating MyNotificationManager object
            MyNotificationManager mNotificationManager = new MyNotificationManager(getApplicationContext());
            Intent intent = null;
            //creating an intent for the notification
            if (title.equals("Task Completed")) {
                intent = new Intent(getApplicationContext(), RequestChangeDisplayActivity.class);
            }

            if (buzzMsg.equals("New Buzz from")) {
                intent = new Intent(getApplicationContext(), BuzzHistoryViewActivity.class);
            }
            if(title.equals("New Task")) {
                intent = new Intent(getApplicationContext(), RequestPendingDisplayActivity.class);
            }
